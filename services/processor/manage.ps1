...
$BASE_NAME = "ayon-aquarium-processor"
...
function defaultfunc {
  Write-Host ""
  Write-Host "*************************"
  Write-Host "AYON aquarium processor service"
...
function build {
  & cp -r "$script_dir/../../client/ayon_aquarium/common/" "$script_dir/aquarium_common"
  try {
    & docker build -t "$IMAGE_FULL_NAME" .
  } finally {
    & Remove-Item -Recurse -Force "$script_dir/aquarium_common"
  }
}
